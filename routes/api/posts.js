...
router.delete('/:id', auth, async (req, res) => {
    try {
        ...
        await post.remove(); return res.json(post);
    }
    catch (e) {
        console.log(e.message);
        if (e.kind == 'ObjectId') return res.status(404).json({ msg: 'Post not found' });
...
router.put('/unlike/:id', auth, async (req, res) => {
    try {
        ...
        post.likes.splice(removeIndex, 1); await post.save(); return res.json(post.likes);
    }
    catch (e) { console.log(e.message); return res.status(500).send('Server Error'); }
});
...
router.post('/comment/:post_id', [auth, [
    ...
    try {
        ...
        post.comments.unshift(newComment); await post.save(); return res.json(post);
    }
    catch (e) { console.log(e.message); return res.status(500).send('Server Error'); }
});
...
router.delete('/comment/:post_id/:comment_id', [auth, [
    ...
    try {
        ...
        post.comments.splice(removeIndex, 1); await post.save(); return res.json(post.comments);
    }
    catch (e) { console.log(e.message); return res.status(500).send('Server Error'); }
});
